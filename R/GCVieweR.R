...
read_gbk <- function(file, sections = NULL, features = NULL, origin = TRUE){

  lines <- readLines(file)

  # Get sections
  section_keys <- if (!is.null(sections)) sections else gbk_get_section_keys(lines)
  section_values <- gbk_get_sections(lines, section_keys)

  # Get Features
  feature_section <- section_values[["FEATURES"]]
  feature_keys <- if (!is.null(features)) features else gbk_get_feature_keys(feature_section)
