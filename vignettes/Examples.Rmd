...
GC_chart(lacZ_operon, group = "Gene_symbol", height = "140px") %>%
  ...
  GC_clusterTitle ("The Lac operon") %>%
  GC_clusterFooter(
    align = "center",
    y = -30,
...
GC_chart(hs_dystrophin_transcripts,
  ...
  GC_scale(
    cluster = c(1, 2, 3, 4),
    start = 1,
    stop = max(hs_dystrophin_transcripts$length),
    hidden = TRUE
  ) %>%
  GC_scale(
    cluster = 5,
    ...
    ticksCount = 8,
    start = 1,
    stop = max(DP_transcripts$length),
  ) %>%
...
GC_chart(erythromycin_cluster, 
  ...
  GC_tooltip(
    formatter = "<b>{Identifiers}</b>
                 <br>{Product}
                 <br>{start} - {stop}({Strand})" 
  ) %>%
  GC_track(track = FALSE) %>%
  GC_scale(
    start = 778000,
    stop = 832000,
    ticksCount = 8
