...
In the below example a title is set for the first cluster and the y position and height is adjusted for the second cluster.
...
GC_chart(ophA_clusters, 
  ...
  GC_title(
    ...
    subtitle = c("<i>Omphalotus Olearius</i>", "<i>Dendrothele Bisporus</i>")
    ) %>%
  GC_labels("name") %>%
...
The `GC_scale()`\` function is utilized for adjusting the chart's scale. The visibility of the scale can be controlled by the hidden parameter, which, when set to TRUE, hides the axis. The `y` parameter can be used to adjust the horizontal position of the axis.
...
By default the minimum and maximum of the genes start and stop positions are used to position the genes along the sequence. With the use of the `start` and `stop parameters, these values can be adjusted. Scale breaks can be added manually using the `breaks` parameter.
...
# With scale breaks
GC_chart(ophA_cluster, group = "class", height = "100px") %>%
  GC_scale(reverse = TRUE)
```

#### Styling

The scale can be styled using the `tickStyle`, `textStyle` and `lineStyle` options. You can pass any valid CSS styles to the respective styling list(). The amount of ticks can be specified using the `ticksCount` option. The `ticksFormat` parameter is used to define how the numerical labels on the tick marks are formatted (see: [link](https://d3js.org/d3-format)).
...
To prevent gene overlaps, genes are automatically separated into distinct tracks. With the `GC_track` function the track-based separation can be switched off or the spacing between tracks can be adjusted.
...
**Note**: The `GC_track()` function must be called after setting genes, labels or coordinates for proper effect.
...
GC_chart(
  ...
  GC_legend(FALSE) %>%
  GC_track(track = FALSE, cluster = 1) %>%
  GC_track(cluster = 2, spacing = 50)
```
