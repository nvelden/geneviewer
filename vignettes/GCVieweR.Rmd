...
To prevent gene overlap, genes can be separated into distinct tracks. In the below example overlapping genes are separated for the second cluster and the spacing between overlapping genes is increased from the default of 40 to 50.
...
GC_chart(
  ...
  GC_legend(FALSE) %>%
  GC_cluster(cluster = 2, prevent_gene_overlap = TRUE, overlap_spacing = 50)
```
