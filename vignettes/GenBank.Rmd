...
The GenBank files from [MIBiG](https://mibig.secondarymetabolites.org/) include additional details, enabling us to categorize and color-code different genes, which we will use to reproduce the visualizations they show on the website. Begin by downloading the GeneBank file from the [MIBiG](https://mibig.secondarymetabolites.org/) website for gene cluster [BGC0000001](https://mibig.secondarymetabolites.org/repository/BGC0000001/index.html#r1c1). Simply click the "Download GenBank file" link and save the file.
...
We can inspect the .gbk using the View pane in Rstudio (use `View(gbk)`) or by simply opening the .gbk file in a text editor. For the visualization we are interested in the start and end position of each gene as well as their identifiers and function. We can find these under FEATURES > CDS. Lets print the first CDS to see what information it contains.   
