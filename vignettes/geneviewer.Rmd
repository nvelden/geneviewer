...
# Generate links
links <- get_links(chart, group = "class")
...
# Add links to chart
chart %>% GC_links(links) %>% GC_labels("name")
```

After generating the links one can add an extra column with identity values. 
...
set.seed(123)
links$identity <- c(40, 90)

# Add links to chart
chart %>% GC_links(links, inverted_color = "#969696", curve = FALSE) %>% GC_legend(TRUE)
```

The links are connected based on the cluster1, cluster2, start1, start2, end1 and end2 values. Which allows you to specify your own boundaries that are not necessary at the start and end of genes.

```{r echo=TRUE, results='asis'}
# Generate links
links <- data.frame(
  start1 = c(30156, 36331),
  end1 = c(32392, 37262),
  cluster1 = c("dbophA", "dbophA"),
  start2 = c(5286, 15046),
  end2 = c(9536, 16016),
  cluster2 = c("ophA", "ophA"),
  identity = c(40, 90)
)
...
webshot2::webshot("temp.html", "output.png", selector = ".geneviewer")
```
