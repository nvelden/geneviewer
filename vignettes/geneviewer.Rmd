...
The `GC_chart()` function can be set to visualize transcripts. It requires setting the transcript column with unique IDs for each transcript and a type column identifying whether the feature is a "UTR", or "exon". Please note that introns are calculated based on exon positions
...
There is a wide range of option to customize the exons, introns and UTRs. For all options see the documentation by running `?GC_transcript()`. 

### Set custom colors for transcripts
