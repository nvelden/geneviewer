...
The `GC_chart()` function can be set to visualize transcripts. It requires setting the transcript column with unique IDs for each transcript and a type column identifying whether the feature is a "UTR", "intron", or "exon".
...
Each transcript can be customized using the `GC_transcript()` function`. For all options see the documentation by running `?GC_transcript()`. 
...
The `itemStyle` in combination with the transcript variable allows for the customization of individual exons, introns, or UTRs within specific transcripts.
